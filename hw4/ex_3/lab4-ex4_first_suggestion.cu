...
int main(int argc, char **argv) {
  ...
  double nzv;                   // Number of non zero values in the sparse matrix
  ...
  int id = 0;
  cusparseDnVecDescr_t temp_dense;
  cusparseDnVecDescr_t tmp_dense;
  ...
  if (concurrentAccessQ) {
    ...
    id = cudaGetDevice(&id);
    cudaMemPrefetchAsync(temp, dimX * sizeof(double), id);
    cudaMemPrefetchAsync(tmp, dimX * sizeof(double), id);
    cudaMemPrefetchAsync(A, nzv * sizeof(double), id);
    cudaMemPrefetchAsync(ARowPtr, (dimX+1) * sizeof(int), id);
    cudaMemPrefetchAsync(AColIndx, nzv * sizeof(int), id);
    cputimer_stop("Prefetching GPU memory to the device");
  }
  cusparseCreateDnVec(&temp_dense, dimX, temp, CUDA_R_32F);
  cusparseCreateDnVec(&tmp_dense, dimX, tmp, CUDA_R_32F);
  cusparseDnVecSetValues(temp_dense, temp);
  cusparseDnVecSetValues(tmp_dense, tmp);
  ...
  cusparseSpMV_bufferSize(cusparseHandle,CUSPARSE_OPERATION_NON_TRANSPOSE, &one, Adescriptor, temp_dense, &zero, temp_dense, CUDA_R_32F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize);
  ...
  for (int it = 0; it < nsteps; ++it) {
    ...
    cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                 &one, Adescriptor,  temp_dense, &zero,  tmp_dense, CUDA_R_32F,
                                 CUSPARSE_SPMV_ALG_DEFAULT, buffer);

    cusparseDnVecGetValues(temp_dense, (void**)&temp);
    cusparseDnVecGetValues(tmp_dense, (void**)&tmp);
