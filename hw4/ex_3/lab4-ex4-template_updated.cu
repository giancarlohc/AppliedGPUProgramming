...
int main(int argc, char **argv) {
  ...
  cusparseCreateDnVec(&temp_dense, dimX, temp, CUDA_R_64F);
  cusparseCreateDnVec(&tmp_dense, dimX, tmp, CUDA_R_64F);
  ...
  cusparseSpMV_bufferSize(cusparseHandle,CUSPARSE_OPERATION_NON_TRANSPOSE, &one, Adescriptor, temp_dense, &zero, temp_dense, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize);
  ...
  for (int it = 0; it < nsteps; ++it) {
    ...
    cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                 &one, Adescriptor,  temp_dense, &zero,  tmp_dense, CUDA_R_64F,
